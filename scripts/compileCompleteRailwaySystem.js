...
console.log('Compiling complete national Indian Railways dataset...');
...
console.log(`Raw records: ${trainsJson.features?.length} trains, ${stationsJson.features?.length} stations, ${schedulesJson.length} schedules`);

// Map stations by code
...
for (const s of schedulesJson) {
  if (!s.train_number) continue;
  const tNo = String(s.train_number).trim();
  if (!schedulesByTrain.has(tNo)) schedulesByTrain.set(tNo, []);
  schedulesByTrain.get(tNo).push(s);
}

// Process and enrich trains
const trainsList = [];
const trainNoSet = new Set();
...
for (const feat of trainsJson.features) {
  ...
  if (trainNoSet.has(trainNo)) continue;
  ...
  let type = 'Express';
  if (trainName.toLowerCase().includes('vande bharat')) type = 'Vande Bharat';
  else if (trainName.toLowerCase().includes('rajdhani')) type = 'Rajdhani';
  else if (trainName.toLowerCase().includes('shatabdi')) type = 'Shatabdi';
  else if (trainName.toLowerCase().includes('duronto')) type = 'Duronto';
  else if (trainName.toLowerCase().includes('garib rath')) type = 'Garib Rath';
  else if (trainName.toLowerCase().includes('jan shatabdi')) type = 'Jan Shatabdi';
  else if (rawType === 'SF' || trainName.toLowerCase().includes('superfast')) type = 'Superfast';
  else if (rawType === 'DEMU' || rawType === 'Pass') type = 'Passenger';

  // Get and format schedule
  let sched = schedulesByTrain.get(rawNo) || schedulesByTrain.get(trainNo) || [];
  sched.sort((a, b) => (a.id || 0) - (b.id || 0));

  let formattedSchedule = [];
  if (sched.length > 0) {
    formattedSchedule = sched.map((s, idx) => {
      ...
      const estDist = Math.round((distance / Math.max(1, sched.length - 1)) * idx);
    ...
    // If no intermediate schedule was in schedules.json, synthesize origin & destination
  ...
  if (p.second_ac) classes['2A'] = { status: 'AVAILABLE', n: 24, fare: Math.max(850, Math.round(distance * 1.9)) };
  if (p.third_ac || type === 'Rajdhani' || type === 'Duronto') classes['3A'] = { status: 'AVAILABLE', n: 56, fare: Math.max(540, Math.round(distance * 1.35)) };
  if (p.chair_car || type === 'Shatabdi' || type === 'Vande Bharat') {
    classes['CC'] = { status: 'AVAILABLE', n: 78, fare: Math.max(380, Math.round(distance * 0.95)) };
    classes['EC'] = { status: 'AVAILABLE', n: 12, fare: Math.max(900, Math.round(distance * 1.8)) };
  }
  if (p.sleeper || Object.keys(classes).length === 0) {
    classes['SL'] = { status: 'AVAILABLE', n: 92, fare: Math.max(175, Math.round(distance * 0.48)) };
  ...
  const dep = (p.departure || formattedSchedule[0]?.dep || '08:00').slice(0, 5);
  const arr = (p.arrival || formattedSchedule[formattedSchedule.length - 1]?.arr || '20:00').slice(0, 5);
...
console.log(`Compiled ${trainsList.length} complete train records!`);

// Save minified datasets
fs.writeFileSync('src/data/allTrainsNational.json', JSON.stringify(trainsList));
fs.writeFileSync('src/data/allStationsNational.json', JSON.stringify(Array.from(stationMap.values())));

console.log('Successfully saved src/data/allTrainsNational.json and src/data/allStationsNational.json!');
